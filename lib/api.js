...
export const api = {
    ...
    // Auth - Now returns Supabase session with JWT
    ...
    getSkills: async (userId) => {
        const headers = await getAuthHeaders();
        const response = await fetch(`${API_BASE_URL}/skills/${userId}`, { headers });
    ...
    getUserStats: async (userId) => {
        const headers = await getAuthHeaders();
        const response = await fetch(`${API_BASE_URL}/dashboard/${userId}`, { headers });
    ...
    getChatHistory: async (userId) => {
        const headers = await getAuthHeaders();
        const response = await fetch(`${API_BASE_URL}/chat/${userId}`, { headers });
    ...
    // ATS report
    getAtsReport: async (userId) => {
        const headers = await getAuthHeaders();
        const response = await fetch(`${API_BASE_URL}/ats/${userId}`, {
            method: 'POST',
            headers,
        });
        const data = await response.json();
        if (!response.ok) {
            throw new Error(data.error || data.details || 'Failed to get ATS report');
        }
        return data;
    },

    getAtsReportLatest: async (userId) => {
        const headers = await getAuthHeaders();
        const response = await fetch(`${API_BASE_URL}/ats/${userId}`, {
            method: 'GET',
            headers,
        ...
        const data = await response.json();
        if (!response.ok) {
            throw new Error(data.error || data.details || 'Failed to fetch ATS report');
        }
        return data;
    }
};
